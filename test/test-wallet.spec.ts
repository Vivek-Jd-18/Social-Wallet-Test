...
import { createWallet, importAnyAccountFromPrivateKey, importAccountFromPhrase, importAccountFromPhraseAndIndex, getBalance } from "../src/wallet-functions/test";
import { networkId } from "../src/wallet-functions/utils";
...
describe("Test getBalance()", async () => {
    ...
    const ethereumResult:any = await getBalance(ethereumAddress,networkId.sepolia);
    console.log(ethereumResult,"BALANCE")
    it("should return the balance of the sepolia(ethereum testnet) address for sepolia(ethereum testnet)", async () => {
        assert.equal(ethereumResult.balance, -1, "should return the balance of given address for sepolia(ethereum testnet)");
    })
    ...
    it("should return the balance of the tron testnet address for tron testnet network", async () => {
        assert.notEqual(tronResult.balance, 0, "should return the balance of given address for tron testnet network");
    })
